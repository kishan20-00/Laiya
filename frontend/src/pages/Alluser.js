...
function ViewUsers() {
    ...
    useEffect(() => {
        function getUsers() {
            axios.get("http://localhost:5300/user/").then((res) => {
                setUsers(res.data);
            }).catch((err) => {
                alert(err.message);
            });
    ...
    const deleteUsers = (id) => {
        axios.delete(`http://localhost:5300/user/delete/${id}`);
    ...
    function sendData(e) {
        ...
        axios.put(`http://localhost:5300/user/update/${updatedValues.id}`, updatedValues)
            .then(() => {
